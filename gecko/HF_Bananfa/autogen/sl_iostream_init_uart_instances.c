...
sl_status_t sl_iostream_uart_init_vcom(void)
{
  ...
  if (!uart_config_vcom.sw_flow_control) {
    #if defined(SL_IOSTREAM_UART_VCOM_FLOW_CONTROL_TYPE)  
    switch (SL_IOSTREAM_UART_VCOM_FLOW_CONTROL_TYPE)
    #elif defined(SL_IOSTREAM_USART_VCOM_FLOW_CONTROL_TYPE)
    switch (SL_IOSTREAM_USART_VCOM_FLOW_CONTROL_TYPE)
    #else 
    #error "Invalid Flow Control Type configuration."
    #endif
    {
  ...
  sl_cos_config_vcom((uint32_t) SL_IOSTREAM_UART_VCOM_BAUDRATE, flow_control);
                           ...
                           sl_power_manager_em_t to)
{
  ...
  if (to < SL_POWER_MANAGER_EM2){
    // Only prepare for wakeup from EM1 or less, since UART doesn't run in EM2
